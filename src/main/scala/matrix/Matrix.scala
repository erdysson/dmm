...
object Matrix {
  ...
  def multiply(vector1: List[Int], vector2: List[Int]): Int = {
  ...
  // todo : use Vector
  def random(size: Int): List[List[Int]] = {
  ...
  def distribute(matrix1: List[List[Int]], matrix2: List[List[Int]]): List[Task] = {
    var seq = 0
    var seqGroup = 0
